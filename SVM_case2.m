function [trainedClassifier, validationAccuracy, F1, AUC,XROC, YROC] = SVM_case2(trainingData, responseData)
...
%
%  Input:
%      trainingData: Feature matrix N x (mean(f), std(f), skewness(f), 
%      mean(h), std(h), sum(h), n_storeys), where N=30 the members of 
%      the population (15 Healthy/15 Damaged), f=frequency and h=height.
%      responseData: Healthy/Damaged label output
%
%  Output:
...
classificationSVM = fitcsvm(...
    ...
    'PolynomialOrder', [], ...
    'KernelScale', 130.6404124396877, ...
    'BoxConstraint', 198.44150450774, ...
    'Standardize', true, ...
    'ClassNames', [0; 1]);
